...
В редактор файлов встроен AI-помощник. Он умеет кратко объяснять выбранный фрагмент, предлагать рефакторинг, генерировать тесты и отвечать на вопросы. Для изменений ассистент возвращает минимальный *unified diff* — его можно предварительно просмотреть и отклонить.
...
Лимиты планов и стоимость операций задаются в файле [`lib/limits.ts`](lib/limits.ts). Здесь указаны ограничения для тарифов и базовые расценки в токенах.
...
5. Проверки качества:
   ...
   pnpm dx:check
   ```
   (линт → тайпчек → тесты). Тесты отдельно:
   ```bash
   pnpm test
   ```
6. CI прогоняет те же шаги, см. [`ci.yml`](.github/workflows/ci.yml).
