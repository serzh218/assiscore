...
export async function createPayment(
  ...
  const description = type === 'PRO_SUBSCRIPTION' ? 'Подписка PRO' : `Пакет токенов ${tokens}`
...
export async function handleWebhook(body: any) {
  ...
  if (status === 'succeeded') {
    if (payment.type === 'PRO_SUBSCRIPTION') {
      ...
      const monthlyTokens = (plan?.features as Features | undefined)?.monthlyTokens ?? 0
      await updateUserPlan(payment.userId, 'PRO')
      if (monthlyTokens > 0) {
        await updateUserTokens(payment.userId, monthlyTokens)
      }
