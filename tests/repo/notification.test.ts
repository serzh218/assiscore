...
import { createNotification, listNotifications, markRead, markAllRead } from '@/server/repo/notification';
...
vi.mock('@/lib/db', () => ({
  prisma: {
    notification: {
      ...
      count: vi.fn(async ({ where }: any) => records.filter((r) => r.userId === where.userId).length),
...
describe('notification repo', () => {
  ...
  it('create and list', async () => {
    await createNotification('u1', { type: NotificationType.GENERATION_READY, title: 't1', body: 'b1' });
    await createNotification('u1', { type: NotificationType.GENERATION_READY, title: 't2', body: 'b2' });
    const res = await listNotifications('u1', { limit: 10, offset: 0 });
