...
vi.mock('@/server/repo/project', () => ({
  getProjectById: vi.fn(async () => ({ id: 'p1', ownerId: 'u1', title: 'Test', type: 'site', spec: {}, visibility: 'public', status: 'draft', createdAt: new Date() })),
  getProjectFiles: vi.fn(async () => ({ 'index.ts': 'content' })),
  updateProjectArtifacts: vi.fn(async (_id: string, { repoUrl }: any) => { savedRepoUrl = repoUrl; }),
}));

vi.mock('@/server/integrations/github', () => ({
  createRepo: vi.fn(async (user: any, { project }: any) => ({ repoUrl: `https://github.com/${user.githubUsername || 'demo'}/${project.title}` })),
...
describe('github export API', () => {
  ...
  it('free user forbidden', async () => {
    ...
    const res = await POST(new Request('http://test', { method: 'POST', body: JSON.stringify({ visibility: 'public' }) }), { params: { id: 'p1' } });
  ...
  it('pro user with github', async () => {
    const { getCurrentUser } = await import('@/lib/auth');
    (getCurrentUser as any).mockResolvedValue({ id: 'u1', plan: 'PRO', githubLinked: true, githubUsername: 'gh' });
    const { POST } = await import('@/app/api/projects/[id]/export/github/route');
    const res = await POST(new Request('http://test', { method: 'POST', body: JSON.stringify({ visibility: 'public' }) }), { params: { id: 'p1' } });
