...
vi.mock('@/server/repo/project', () => ({
  ...
  setProjectFiles: vi.fn(async (_id: string, f: Record<string, string>) => {
    files = f
  }),
...
vi.mock('@/server/repo/patch', () => ({
  createPatch: vi.fn(async (data: any) => {
    patches.push(data)
    return {
      id: 'patch1',
...
describe('patch pipeline API', () => {
  ...
  it('applies diff and stores patch', async () => {
    ...
    const req = new Request('http://test/api/projects/p1/patch', {
      method: 'POST',
      body: JSON.stringify({ message: 'upd' }),
    })
