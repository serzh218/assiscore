...
const notifications = [
  { id: 'n1', userId: 'u1', type: 'GENERATION_READY', title: 't1', body: 'b1', createdAt: new Date() },
  { id: 'n2', userId: 'u1', type: 'GENERATION_READY', title: 't2', body: 'b2', createdAt: new Date() },
  { id: 'n3', userId: 'u2', type: 'GENERATION_READY', title: 'x', body: 'x', createdAt: new Date() },
];
...
describe('notifications API', () => {
  ...
  it('POST read marks specific ids', async () => {
    ...
    await POST(new Request('http://test/api/notifications/read', { method: 'POST', body: JSON.stringify({ ids: ['n1'] }) }));
  ...
  it('POST read without ids marks all', async () => {
    const { POST } = await import('@/app/api/notifications/read/route');
    await POST(new Request('http://test/api/notifications/read', { method: 'POST', body: JSON.stringify({}) }));
    expect(notifications[0].readAt).toBeInstanceOf(Date);
