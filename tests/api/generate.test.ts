...
vi.mock('@/server/repo/project', () => ({
  ...
  updateProjectStatus: vi.fn(async (_id: string, status: string) => { projectStatus = status; return { ...project, status }; })
}));

vi.mock('@/server/repo/generation', () => ({
  createGeneration: vi.fn(async () => ({ id: 'g1', projectId: 'p1', costTokens: 10, logs: '', createdAt: new Date() })),
  appendGenerationLogs: vi.fn(async (_id: string, chunk: string) => { genLogs += chunk; return { id: 'g1', projectId: 'p1', costTokens: 10, logs: genLogs, createdAt: new Date() }; }),
...
describe('generate API', () => {
  ...
  it('creates project and returns status with logs', async () => {
    ...
    const res = await POST(new Request('http://test/api/generate', { method: 'POST', body: JSON.stringify({ prompt: 'Hello' }) }));
