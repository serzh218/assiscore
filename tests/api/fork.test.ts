...
vi.mock('@/server/repo/project', () => ({
  getProjectById: vi.fn(async () => ({ id: 'p1', ownerId: 'u1', visibility: 'public', title: '', type: '', spec: {}, status: 'draft', createdAt: new Date() })),
  forkProject: vi.fn(async ({ newOwnerId }: any) => ({ id: 'p2', ownerId: newOwnerId, visibility: 'public', title: '', type: '', spec: {}, status: 'draft', createdAt: new Date() })),
}));
