...
export async function GET(_req: Request, { params }: { params: { id: string; patchId: string } }) {
  ...
  const patch = await getPatchById(params.patchId);
...
export async function DELETE(_req: Request, { params }: { params: { id: string; patchId: string } }) {
  ...
  await deletePatch(params.patchId);
