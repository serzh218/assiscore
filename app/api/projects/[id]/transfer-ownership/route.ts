...
export async function POST(
  ...
  try {
    await assertProjectPermission(params.id, user.id, 'project:transfer');
  } catch (e: any) {
  ...
  try {
    await setOwner(params.id, newOwnerUserId);
    await writeAudit(params.id, user.id, 'owner.transfer', {
      from: user.id,
      to: newOwnerUserId,
    });
