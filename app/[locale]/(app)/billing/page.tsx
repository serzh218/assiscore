    ...
    <div className="space-y-8">
      <Card>
        ...
        <CardContent className="space-y-2">
          <p>
            Неограниченные приватные проекты, интеграция с GitHub, публикация на домены.
          </p>
      ...
      <Card>
        ...
        <CardContent>
          {user.plan === 'PRO' && (
            <p className="mb-2 text-green-600">План PRO активен</p>
          )}
          <Table headers={["Тип", "Сумма", "Статус", "Дата"]}>
            {payments.map((p) => (
              <tr key={p.id}>
                <td className="py-2">
                  {p.type === 'PRO_SUBSCRIPTION'
                    ? 'Подписка PRO'
                    : `Пакет токенов ${p.tokens}`}
                </td>
